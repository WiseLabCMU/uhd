// A test for timed reception from a USRP N210. Print out the current USRP time,
// then start acquisition for the next second epoch. i.e. if current time is
// s.mmm, acquire samples between (s+1).000 to (s+2).000. Note that there will
// be some cases where s.mmm is too close to the epoch. In this case, return a
// too-late error Based on the rx_timed_samples code in Ettus examples Adwait
// Dongare
...
// Adwait: safe main catches exceptions and prints them to the error console
